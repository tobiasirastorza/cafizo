  ...
  return (
    <aside className="relative flex min-h-screen w-full flex-col justify-between border-r-2 border-border bg-black px-6 py-10 md:sticky md:top-0 md:w-[260px]">
      <div>
        ...
        <div className="mt-1 text-base font-bold uppercase tracking-widest text-accent">
          v2.0.4
        </div>

        <nav className="mt-12 space-y-8 text-lg font-bold uppercase tracking-widest">
            ...
            return (
              ...
              >
                <span className="inline-flex h-6 w-6 items-center justify-center">
                  ...
                  {link.key === "students" && <RiUserLine className="h-5 w-5" />}
      ...
      <div className="space-y-6">
        ...
        <Link
          ...
          className={`flex items-center gap-4 text-lg font-bold uppercase tracking-widest ${
            isActiveLink(links[4].href) ? "text-accent" : "text-muted-foreground"
          }`}
        >
          <span className="inline-flex h-6 w-6 items-center justify-center">
            <RiBarChartBoxLine className="h-5 w-5" />
          </span>
          <span>{t(links[4].key)}</span>
        </Link>
      </div>
    </aside>
  );
